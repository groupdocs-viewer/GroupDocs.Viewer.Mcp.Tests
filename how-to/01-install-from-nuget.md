...
Two install routes from [`nuget.org/packages/GroupDocs.Viewer.Mcp`](https://www.nuget.org/packages/GroupDocs.Viewer.Mcp):
...
```bash
dnx GroupDocs.Viewer.Mcp --yes                # latest stable, refreshed every run
...
The cache lives at `%USERPROFILE%\.nuget\packages\groupdocs.viewer.mcp\<version>\`
on Windows and `~/.nuget/packages/groupdocs.viewer.mcp/<version>/` on
...
info: ModelContextProtocol.Server.StdioServerTransport[...]
      Server (stream) (GroupDocs.Viewer.Mcp) transport reading messages.
...
(
  ...
  sleep 2
) | GROUPDOCS_MCP_STORAGE_PATH=./docs dnx GroupDocs.Viewer.Mcp@26.7.0 --yes
```
...
```bash
dotnet tool update -g GroupDocs.Viewer.Mcp
```

To uninstall:

```bash
dotnet tool uninstall -g GroupDocs.Viewer.Mcp
```

The tool runs from `~/.dotnet/tools/groupdocs-viewer-mcp` (Linux/macOS) or
`%USERPROFILE%\.dotnet\tools\groupdocs-viewer-mcp.exe` (Windows). Make sure
...
| `GROUPDOCS_LICENSE_PATH` | Path to `GroupDocs.Total.lic` — required for `render_page` | *(evaluation mode)* |
...
`get_view_info` works fine in evaluation mode. `render_page` requires a
license — without one, the underlying `GroupDocs.Viewer.Save()` call throws
`RenderPage` output is watermarked.

Get a license from [purchase.groupdocs.com](https://purchase.groupdocs.com/).
Point `GROUPDOCS_LICENSE_PATH` at the `.lic` file and `get_view_info` output
...
```text
initialize response → serverInfo: { name: "GroupDocs.Viewer.Mcp", version: "26.7.0" }
```
...
| `No license configured. Running in evaluation mode.` | No `GROUPDOCS_LICENSE_PATH` | Expected. `RenderPage` and `GetViewInfo` both still work; `RenderPage` output is watermarked. Set the path to drop the watermark. |
