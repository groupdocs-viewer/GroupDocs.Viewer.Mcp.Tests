...
2. Open Copilot chat.
3. Click the tools / MCP icon — `groupdocs-viewer` should appear with
   `get_view_info` and `render_page` listed.
...
The [NuGet.org package page](https://www.nuget.org/packages/GroupDocs.Viewer.Mcp)
...
```
@workspace render page 1 of /docs/contract.pdf

Use get_view_info to compare the authors of old.docx and new.docx.

Render the first 3 pages of presentation.pptx -- show me the images.
```
...
Without a license, `render_page` errors (see
