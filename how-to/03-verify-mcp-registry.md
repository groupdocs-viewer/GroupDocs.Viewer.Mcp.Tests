...
The `GroupDocs.Viewer.Mcp` package is an **MCP server** (the csproj declares
...
[nuget.org/packages/GroupDocs.Viewer.Mcp](https://www.nuget.org/packages/GroupDocs.Viewer.Mcp)
...
3. Search for `groupdocs-viewer` or `viewer`.
4. The package should appear with the description from `server.json`:
   *"MCP server for GroupDocs.Viewer for .NET — render document pages as PNG images, plus inspect view info (page count, dimensions, file type). Supports PDF, Word, Excel, PowerPoint, ODT, RTF, HTML, and 170+ more formats. Exposes RenderPage and GetViewInfo tools to AI agents."*
...
curl -s https://registry.modelcontextprotocol.io/v0/servers \
  | jq '.[] | select(.name | contains("groupdocs-viewer"))'
```
...
- `ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsViewerMcp` — the server
  reports `serverInfo.name == "GroupDocs.Viewer.Mcp"` and a non-empty version
  that matches the package under test.
- `ToolDiscoveryTests.ListTools_ExposesReadAndRenderPage` — both MCP tools
  are advertised.
