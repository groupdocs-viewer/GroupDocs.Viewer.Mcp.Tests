...
This repo's test suite validates the **published** `GroupDocs.Viewer.Mcp`
...
Three tests in `RenderPageTests` only assert success when a GroupDocs
...
# Only RenderPage tests
dotnet test -c Release --filter "FullyQualifiedName~GetViewInfo"
...
Passed  RenderPageTests.RenderPage_Jpeg_WritesCleanOutput_Licensed            (no-op without license)
Passed  RenderPageTests.RenderPage_FollowUpReadOfCleanedFile_Succeeds_Licensed (no-op without license)
...
Synthetic PDF/JPEG fixtures ([SampleDocuments.cs](../src/GroupDocs.Viewer.Mcp.Tests/Fixtures/SampleDocuments.cs))
...
- name: Dispatch integration tests
  ...
  run: |
    gh api \
      repos/groupdocs-viewer/GroupDocs.Viewer.Mcp.Tests/dispatches \
...
base64 -w0 GroupDocs.Total.lic | gh secret set GROUPDOCS_LICENSE \
  --repo groupdocs-viewer/GroupDocs.Viewer.Mcp.Tests
```
...
test fails with a cryptic `"An error occurred invoking 'render_page'"`,
...
(
  ...
  echo '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"render_page","arguments":{"file":{"filePath":"some.pdf"}}}}'
  sleep 5
) | GROUPDOCS_MCP_STORAGE_PATH=/tmp/gd dnx GroupDocs.Viewer.Mcp@26.5.0 --yes \
...
| All RenderPage tests fail | Running unlicensed. Expected — `RenderPage_InEvaluationMode_ReturnsErrorResponse` should pass, the `*_Licensed` tests no-op. If the eval-mode test also fails, check the package actually installed. |
