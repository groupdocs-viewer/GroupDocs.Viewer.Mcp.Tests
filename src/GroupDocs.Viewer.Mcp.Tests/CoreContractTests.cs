...
public class CoreContractTests : IClassFixture<McpServerFixture>
{
    ...
    public async Task InfoTool_WithFileNameOnly_Resolves()
    {
        ...
        var response = await _fixture.Client.CallToolAsync(
            ...
            {
                ["file"] = new Dictionary<string, object?> { ["fileName"] = SampleDocuments.AuthoredPdf },
            });
    ...
    public async Task InfoTool_WithNoArguments_NamesTheMissingParameter()
    {
        ...
        var response = await _fixture.Client.CallToolAsync(
            catalog.ViewInfo.Name, new Dictionary<string, object?>());
    ...
    public async Task GetLicenseStatus_IsRegisteredAndDescribesTheServer()
    {
        ...
        Assert.Equal("GroupDocs.Viewer", engine.GetProperty("name").GetString());
