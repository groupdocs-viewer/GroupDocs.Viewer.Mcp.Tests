...
Complete bash script suite for running GroupDocs.Viewer.Mcp integration tests in both local and Docker environments. **Works on Windows (Git Bash/WSL), Linux, and macOS.**

## 📋 Overview

This folder contains production-ready bash scripts for testing the GroupDocs.Viewer.Mcp NuGet package across all scenarios:

- **ToolDiscovery** — MCP server handshake and tool listing
- **RenderPage** -- Render document pages as PNG images (PDF/Word/Excel/PowerPoint/170+ formats), returns image inline + saves to storage
- **GetViewInfo** -- File type, page count, and per-page dimensions for source documents (no rendering)
...
| `04_run-server-with-samples.sh` | **Interactive Docker MCP server** — Launches the published `viewer-net-mcp` container with `sample-docs/` mounted, for ad-hoc smoke testing. |
...
./02_test-all-scenarios.sh --filter GetViewInfo
...
**Output:**
```
Passed  ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsViewerMcp
Passed  GetViewInfoTests.GetViewInfo_AuthoredPdf_ReturnsFileFormatAndProperties
Passed  ErrorHandlingTests.GetViewInfo_UnknownFile_ReturnsErrorListingAvailableFiles
...
...
services:
  viewer-server:
    image: ghcr.io/groupdocs-viewer/viewer-net-mcp:latest
    volumes:
...
```bash
# Unlock licensed-mode RenderPage tests
...
Without a license:
- `RenderPage_InEvaluationMode_ReturnsErrorResponse` — ✅ Passes (asserts graceful error)
- `RenderPage_Jpeg_WritesCleanOutput_Licensed` — ⏭️ Skipped
- `RenderPage_FollowUpReadOfCleanedFile_Succeeds_Licensed` — ⏭️ Skipped
...
| `GetViewInfo` | 3 tests | `./02_test-all-scenarios.sh --filter GetViewInfo` |
| `RenderPage` | 3 tests | `./02_test-all-scenarios.sh --filter RenderPage` |
...
docker build -t viewer-net-mcp:local ../../../GroupDocs.Viewer.Mcp/
...
```bash
for version in 26.7.0 26.7.0 latest; do
...
# Docker utilities
docker_pull_image "ghcr.io/groupdocs-viewer/viewer-net-mcp:latest"
check_docker_daemon
...
```
Passed  ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsViewerMcp
Passed  ToolDiscoveryTests.ListTools_ExposesReadAndRenderPage
Passed  ToolDiscoveryTests.AllTools_HaveNonEmptyDescriptionAndInputSchema
Passed  GetViewInfoTests.GetViewInfo_AuthoredPdf_ReturnsFileFormatAndProperties
Passed  GetViewInfoTests.GetViewInfo_Jpeg_ReturnsJpegFormat
Passed  GetViewInfoTests.GetViewInfo_AuthoredPdf_SurfacesKnownAuthorValue
Passed  RenderPageTests.RenderPage_InEvaluationMode_ReturnsErrorResponse
Passed  RenderPageTests.RenderPage_Jpeg_WritesCleanOutput_Licensed
Passed  RenderPageTests.RenderPage_FollowUpReadOfCleanedFile_Succeeds_Licensed
Passed  ErrorHandlingTests.GetViewInfo_UnknownFile_ReturnsErrorListingAvailableFiles
Passed  ErrorHandlingTests.GetViewInfo_CorruptedFile_DoesNotCrashServer
...
- [GroupDocs.Viewer.Mcp NuGet](https://www.nuget.org/packages/GroupDocs.Viewer.Mcp)
- [Integration Tests Guide](../how-to/06-run-integration-tests.md)
- [Docker Setup Guide](../how-to/02-run-via-docker.md)
- [Test Project](../src/GroupDocs.Viewer.Mcp.Tests/GroupDocs.Viewer.Mcp.Tests.csproj)
