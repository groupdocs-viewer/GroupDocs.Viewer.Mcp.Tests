...
#
# Launches `ghcr.io/groupdocs-viewer/viewer-net-mcp` as an interactive MCP
...
#                           advertise both `get_view_info` and `render_page`.
#   --license PATH          Mount a GroupDocs license file (enables
#                           render_page; without it the server runs in
#                           evaluation mode)
...
IMAGE_REPO="ghcr.io/groupdocs-viewer/viewer-net-mcp"
...
# nothing to /data when only running get_view_info / non-licensed render_page.
...
if [ -n "$LICENSE_PATH" ]; then
    ...
    log_info "License mounted — render_page will work in licensed mode"
else
    log_info "No license — render_page will return evaluation-mode error (get_view_info works fine)"
fi
...
if [ "$RUN_MODE" = "smoke" ]; then
    ...
    if echo "$response" | grep -q '"get_view_info"' && echo "$response" | grep -q '"render_page"'; then
